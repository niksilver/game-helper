...
class TestOptimise:
    """Tests for the optimise() static method."""
    ...
    def test_different_guesses_find_same_answer(self):
        ...
        assert CardMaker.optimise(1,    assess) == target
        assert CardMaker.optimise(10,   assess) == target
        assert CardMaker.optimise(73,   assess) == target
        assert CardMaker.optimise(200,  assess) == target
        assert CardMaker.optimise(1000, assess) == target
    ...
    def test_no_exact_match_returns_best_acceptable(self):
        ...
        result = CardMaker.optimise(3, assess)
        assert 10 <= result <= 15

        # Initial guess above the acceptable range
        result = CardMaker.optimise(50, assess)
        assert 10 <= result <= 15
